...
export async function GET(req: NextRequest) {
  ...
  if (error || !code || !state) {
    return NextResponse.redirect(new URL("/dashboard/blogs?error=google_oauth", baseUrl));
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/dashboard/blogs?connected=1", baseUrl));
  } catch (e) {
    console.error(e);
    return NextResponse.redirect(new URL("/dashboard/blogs?error=google_oauth", baseUrl));
  }
